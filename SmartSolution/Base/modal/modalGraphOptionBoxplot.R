...
ModalGraphOptionBoxplot <- function(failed = FALSE) {
  labelStr <- "그래프 옵션"
  # failed <- FALSE
  ...
  modalDialog(
    ...
    footer = tagList(
      modalButton("Cancel"),
      actionButton("okModalGraphOptionBoxplot", "OK")
    ),
    size="l"
  )
}

treatModalGraphOptionBoxplot <- function(input, output, session) {


  observeEvent(input$okModalGraphOptionBoxplot, {
