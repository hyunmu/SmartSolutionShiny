modYVF1TabModuleUI <- function(Id) {
    ...
    fluidPage(
        ...
        fluidRow(
            ...
            column(1, actionButton(ns("MCP_x"), "VF 변수 선정") 
            ),
...
modYVF1TabModule <- function(input, output, session) {
    ns <-session$ns
    ...
    observeEvent(input$MCP_color, {
        var <- setdiff(unique(colnames(curSampleExplore)), dfModelNest[["modelDirectRaw"]][[1]])
        var <- c("NULL",var)
        curAes <<- "color"
    ...
    observeEvent(input$graphUpdate, {
        ...
        # refDF <- curSampleExplore[,c(dfModelNest[["modelDirectRaw"]][[1]], "predVal", "resid")]
        refDF <- curSampleExplore
        
        for(item in dfModelNest[["modelDirectRaw"]][[1]]) {
            refDF[,item] <- attr(curSampleExplore[,item],"mean")
            # colnames(refDF)[i] <- aesList[["x"]][i]
        }
        
        refDF[,x] <- curSampleExplore[,x]
        dfGraph <- add_predictions(refDF, curSelModel, var="modY")
        dfGraph[,"modY"] <- dfGraph[,"modY"] + curSampleExplore[,"resid"] 
        
        attr(dfGraph[,y], "max") <- max(dfGraph[,y], na.rm=TRUE)
        attr(dfGraph[,y], "mean") <- mean(dfGraph[,y], na.rm=TRUE)
        attr(dfGraph[,y], "min") <- min(dfGraph[,y], na.rm=TRUE)
        attr(dfGraph[,y], "label") <- attr(DFSource[,y], "label")
        ## 개발자용 : 특정 모델로 다른 도메인 데이터를 검증할 때 필요
        # attr(df[,"modY"], "label") <- 
        #   paste0("예측", attr(DFSource[,dfModelNest[["modelY"]]], "label")) 

        graphOption[["minY"]][1]<<- attr(dfGraph[,y], "min")
        graphOption[["maxY"]][1]<<- attr(dfGraph[,y], "max")
        
        attr(dfGraph[,x], "max") <- max(dfGraph[,x], na.rm=TRUE)
        attr(dfGraph[,x], "mean") <- mean(dfGraph[,x], na.rm=TRUE)
        attr(dfGraph[,x], "min") <- min(dfGraph[,x], na.rm=TRUE)
        attr(dfGraph[,x], "label") <- attr(DFSource[,x], "label")
        ## 개발자용 : 특정 모델로 다른 도메인 데이터를 검증할 때 필요
        # attr(df[,"modY"], "label") <- 
        #   paste0("예측", attr(DFSource[,dfModelNest[["modelY"]]], "label")) 
        
        graphOption[["minX"]][1]<<- attr(dfGraph[,x], "min")
        graphOption[["maxX"]][1]<<- attr(dfGraph[,x], "max")
